...
class UKAN(nn.Module):
    ...
    drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[1, 1, 1], **kwargs):
        ...
        if self.deep_supervision:
            # Two auxiliary heads similar to model/model.py: one after decoder2, one after decoder3.
            self.ds_near = nn.Conv2d(embed_dims[0], num_classes, kernel_size=1)        # after decoder2 (skip t3)
            self.ds_far = nn.Conv2d(embed_dims[0]//4, num_classes, kernel_size=1)      # after decoder3 (skip t2)
    ...
    def forward(self, x):
        ...
        if self.deep_supervision:
            ds_near = F.interpolate(self.ds_near(out), size=input_size, mode='bilinear', align_corners=False)
        _,_,H,W = out.shape
        out = out.flatten(2).transpose(1,2)
        ...
        if self.deep_supervision:
            ds_far = F.interpolate(self.ds_far(out), size=input_size, mode='bilinear', align_corners=False)
        ...
        if self.deep_supervision and self.training:
            # Order aligns with model/model.py: [main, aux_near, aux_far]
            return [final_out, ds_near, ds_far]
        return final_out
